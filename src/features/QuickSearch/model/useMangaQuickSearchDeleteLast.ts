...
export function useMangaQuickSearchDeleteLast() {
    const queryClient = useQueryClient();
    const [optimisticDeleted, setOptimisticDeleted] = useOptimistic<string[]>([]);
    const { mutateAsync } = authRqClient.useMutation('delete', '/quick-search/manga/last', {
        onSuccess: () => {
            queryClient.invalidateQueries(
                authRqClient.queryOptions('get', '/quick-search/manga/last'),
            );
        },
    });
